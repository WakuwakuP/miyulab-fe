...
| 3     | [プロフィール正規化](03-profiles.md)                             | v8 → v9    | 🔲 未着手 | Phase 2       |
| 4     | [投稿データ正規化](04-post-normalization.md)                     | v9 → v10   | 🔲 未着手 | Phase 1, 2    |
| 5     | [エンゲージメント統一](05-engagements.md)                        | v10 → v11  | 🔲 未着手 | Phase 2, 3, 4 |
| 6     | [マテリアライズドビュー見直し](06-materialized-view-refactor.md) | v11 → v12  | 🔲 未着手 | Phase 1, 4    |
| 7     | [JSON blob 廃止](07-json-elimination.md)                         | v12 → v13  | 🔲 未着手 | Phase 1〜6    |
| 8     | [タイムライン再構築](08-timeline-restructure.md)                 | v13 → v14  | 🔲 未着手 | Phase 1〜7    |
...
- [ ] Step 1: テーブル作成 (`profiles`, `profile_aliases`, `profile_fields`, `local_accounts`)
- [ ] Step 2: 既存データからプロフィールを抽出 (JSON → `profiles`)
- [ ] Step 3: `profile_aliases` の生成
- [ ] Step 4: `posts` に `author_profile_id` カラム追加・バックフィル
- [ ] Step 5: `notifications` に `actor_profile_id` カラム追加・バックフィル
- [ ] Step 6: `home_server_id` の補完
- [ ] アプリケーション層の変更
  - [ ] `schema.ts` — `SCHEMA_VERSION = 9`, `migrateV8toV9()` 追加
  - [ ] `shared.ts` — `ensureProfile(db, account)` ヘルパー追加
  - [ ] `workerStatusStore.ts` — upsert 時に profiles UPSERT 追加
  - [ ] `workerNotificationStore.ts` — 同上
  - [ ] `statusStore.ts` — クエリに profiles JOIN 追加
- [ ] テスト・検証
...
- [ ] Step 1: テーブル作成
  ...
  - [ ] `custom_emojis`
  - [ ] `polls` / `poll_options`
  - [ ] `link_cards` / `post_links`
- [ ] Step 2: `post_media` のバックフィル (JSON → `post_media`)
- [ ] Step 3: `hashtags` / `post_hashtags` のバックフィル
- [ ] Step 4: `post_stats` のバックフィル
- [ ] Step 5: `polls` / `poll_options` のバックフィル
- [ ] Step 6: `link_cards` / `post_links` のバックフィル
- [ ] アプリケーション層の変更
  - [ ] `schema.ts` — `SCHEMA_VERSION = 10`, `migrateV9toV10()` 追加
  - [ ] `workerStatusStore.ts` — 投稿保存時にサブテーブルへ分離保存
- [ ] テスト・検証
...
- [ ] Step 1: `post_engagements` テーブル作成
- [ ] Step 2: 既存データのバックフィル (JSON フラグ → `post_engagements`)
  - [ ] favourite
  - [ ] reblog
  - [ ] bookmark
...
- [ ] アプリケーション層の変更
  ...
  - [ ] `workerStatusStore.ts` — `toggleEngagement()` ヘルパー追加
  - [ ] `statusStore.ts` — ブックマーク一覧クエリ追加
  - [ ] `queryBuilder.ts` — `pe` エイリアス追加
- [ ] テスト・検証
...
- [ ] Step 1: 現在のタイムラインクエリの `timeline_entries` / `tag_entries` 参照を確認
- [ ] Step 2: 代替インデックスの作成
- [ ] Step 3: トリガーの削除 (7つ)
- [ ] Step 4: `timeline_entries` / `tag_entries` テーブル削除
- [ ] Step 5: クエリの書き換え (JOIN 方式へ)
- [ ] Step 6: EXPLAIN QUERY PLAN による検証
- [ ] アプリケーション層の変更
  - [ ] `schema.ts` — `SCHEMA_VERSION = 12`, マテビュー関連関数削除
  - [ ] `statusStore.ts` — JOIN 方式に書き換え
  - [ ] `workerStatusStore.ts` — トリガー依存コード削除
- [ ] テスト・検証
  - [ ] bulk insert の性能改善確認
...
- [ ] Step 1: `posts` に不足カラム追加 (`content_html`, `spoiler_text`, `is_local_only`, `edited_at`, `canonical_url`)
- [ ] Step 2: 新カラムへのバックフィル (`json_extract` → 正規化カラム)
- [ ] Step 3: `notifications` に不足カラム追加 (`related_post_id`)
- [ ] Step 4: `json` カラム廃止 — テーブル再構築 (`posts_v13`, `notifications_v13`)
- [ ] Step 5: テーブル置き換え (DROP → RENAME)
- [ ] Step 6: インデックス再作成
- [ ] Step 7: 不要テーブルの削除 (`posts_belonging_tags` 等)
- [ ] アプリケーション層の変更（大規模）
  - [ ] `statusStore.ts` — `buildEntityStatus()` 組み立て関数の新設
  - [ ] `notificationStore.ts` — `buildEntityNotification()` 組み立て関数の新設
  - [ ] `workerStatusStore.ts` — 全ハンドラから JSON 文字列操作を削除
  - [ ] `workerNotificationStore.ts` — 同上
  - [ ] `shared.ts` — `extractStatusColumns` / `extractNotificationColumns` 削除
  - [ ] `queryBuilder.ts` — `json_extract` 参照を削除
  - [ ] 型定義の更新 (`SqliteStoredStatus` 等)
- [ ] テスト・検証
...
- [ ] Step 2: データ移行
  - [ ] `timelines` の初期生成
  - [ ] `timeline_items` へのデータ移行
  - [ ] `feed_events` 初期データ
- [ ] Step 3: アプリケーション層の改修
  - [ ] `workerStatusStore.ts` — タイムライン帰属の書き込み変更
  - [ ] `statusStore.ts` — タイムライン読み取りの変更
  - [ ] `queryBuilder.ts` — `timeline_id` ベースに変更
  - [ ] `feed_events` の書き込みロジック追加
- [ ] Step 4: 旧テーブルの廃止 (`statuses_timeline_types` 等)
- [ ] Step 5: クリーンアップ処理の更新 (`workerCleanup.ts`)
- [ ] テスト・検証
