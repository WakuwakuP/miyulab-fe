...
describe('itemKey', () => {
  it('post_id があれば p:{id} を返す', () => {
    const item = { created_at_ms: 100, id: 'x', post_id: 42 } as any
    expect(itemKey(item)).toBe('p:42')
  })

  it('notification_id があれば n:{id} を返す', () => {
    const item = { created_at_ms: 200, id: 'y', notification_id: 7 } as any
    expect(itemKey(item)).toBe('n:7')
  })

  it('それ以外は u:{id} を返す', () => {
    const item = { created_at_ms: 300, id: 'abc' } as any
...
describe('itemTimestamp', () => {
  it('created_at_ms があればそのまま返す', () => {
    const item = { created_at_ms: 1000, id: '1' } as any
    expect(itemTimestamp(item)).toBe(1000)
  })

  it('created_at (ISO文字列) からパースする', () => {
    const item = { created_at: '2024-01-01T00:00:00.000Z', id: '1' } as any
...
describe('mergeItemsIntoMap', () => {
  ...
  it('既存アイテムを上書きする', () => {
    ...
    expect((map.get('p:1') as any).content).toBe('new')
  })
})
...
describe('sortItemsDesc', () => {
  it('created_at_ms の降順でソートする', () => {
    ...
    expect(sorted.map((i: any) => i.created_at_ms)).toEqual([300, 200, 100])
  })
