...

{
  ...
  // フォロー
  followsOnly?: boolean        // フォロー中のアカウントのみ（※v2 スキーマでは未実装）
}
...
- **OR モード**: `WHERE ht.name IN ('tag1', 'tag2')` + `GROUP BY p.id`
- **AND モード**: `HAVING COUNT(DISTINCT ht.name) = タグ数`
...
**安全対策**: `useCustomQueryTimeline` がサニタイズ処理を行い、DML/DDL/PRAGMA 等を禁止、セミコロン除去、LIMIT/OFFSET/SQL コメント除去を行う。
...
1. **customQuery の除去**: `advancedQuery` が未設定のタイムラインから `customQuery` を削除（`cleanupNonAdvancedCustomQuery`）。通常 UI モード時に不要な重い LEFT JOIN クエリが実行されるのを防ぐ。
...
|------|-------|------|
| `home` | ホーム | デフォルト設定 |
| `notification` | 通知 | `notificationFilter: ALL_NOTIFICATION_TYPES` |
| `tag` | ごちそうフォト | `tagConfig: { mode: 'or', tags: ['ごちそうフォト'] }`, `onlyMedia: true` |
| `public` | パブリック(メディア) | `onlyMedia: true` |

## 表示名の自動生成

`timelineDisplayName.ts` の `getDefaultTimelineName()` がフィルタ設定に応じて表示名を自動生成する。
...
| 関数 | 処理 |
|------|------|
| `normalizeBackendFilter` | 存在しない backendUrl を除去、composite 1件を single に縮退、URL ソート |
